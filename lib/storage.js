...
const GenericStorage = function(Storage,StorageLocator) {
    ...
    this.getResourceName = ( location, resName, resType, options ) => {
        ...
        let resourceName 
        if ( typeof storageLocator.getResourceName == "function" ){
            resourceName = storageLocator.getResourceName( location, resName, resType, options )
        }
        else {
            if ( resType == "key" ){        
    ...
    this.storePkiResource = async function( location,resName,resType,data,options ) {
        ...
        return storage.storePkiResource( location,resourceName,data )
    }

    this.dumpPkiResource = async function( location,resName,resType,options ) {
        const resourceName = this.getResourceName(location,resName,resType,options )
