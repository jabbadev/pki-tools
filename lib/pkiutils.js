...
import { CertificateAttributes, CertificateIn, CertificateInput, CertificateIssuer, CertificateSubject, CsrAttributes, certificateInputHandler, respondTo } from './commons.js';
...
export const generateCaRootCertificate = (subject,serialNum,options) => new Promise( async (resolve,reject)=>{
  ...
  try {
    const keys = await generateRsaPemKeys({ bits: options.keySize })
    ...
    resolve({ ...keys, certificate: caCert })
  }
  catch( error ){
    reject(error)
  }
